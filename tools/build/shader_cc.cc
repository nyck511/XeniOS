...
int main(int argc, char** argv) {
  ...
  while (arg_idx < argc && argv[arg_idx][0] == '-') {
    if (std::strcmp(argv[arg_idx], "--msl") == 0) {
      ...
      arg_idx += 2;
               ...
               std::strcmp(argv[arg_idx], "--metal-std") == 0 ||
               std::strcmp(argv[arg_idx], "--metal-min-version-flag") == 0) {
      if (arg_idx + 1 >= argc) {
        std::fprintf(stderr, "%s requires a value\n", argv[arg_idx]);
        return 1;
      }
      std::string* destination =
          std::strcmp(argv[arg_idx], "--metal-sdk") == 0
              ? &metal_sdk
              : (std::strcmp(argv[arg_idx], "--metal-std") == 0
                     ? &metal_std
                     : &metal_min_version_flag);
      *destination = argv[arg_idx + 1];
      arg_idx += 2;
    } else {
  ...
  if (slang_dxil_mode || slang_spirv_mode || slang_msl_mode) {
    ...
    if (RunCommand({"xcrun", "-sdk", metal_sdk, "metal", "-x", "metal",
                    "-std=" + metal_std, metal_min_version_flag, "-w", "-c",
